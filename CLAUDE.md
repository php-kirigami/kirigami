...
- **`@kirigami/kirigami` 1.4.3** — banner-from-template: `config.js` fills all
  ...
  fallback (`files` gained `assets/`); `kiri create` `--email` / `--repo` +
  writes a starter `banner.txt`. Both `../template-*/banner.txt` swapped to the
  token version. 1.4.3 also dep-bumps `@kirigami/php-prepros` to 1.7.2.
...
Release when ready: `npm run release` (publishes php-prepros 1.7.2 + kiri 1.4.3 +
plugin-highlight 0.1.3; everything else skips). Topo order: sdk → canva →
