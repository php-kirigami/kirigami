...
**Fifth release shipped (2026-09-11).** On npm and good: `@kirigami/canva`
**2.6.0**, `@kirigami/kirigami` **1.5.7**, `@kirigami/plugin-highlight`
**0.1.7**, `@kirigami/plugin-embed` **0.1.5** (`php-prepros`/`sdk`/
`struct-walker`/`plugin-extlink`/`php-wasm` unchanged, skipped by the release
script). The feature: canva's **`styles/lightswitch`** — an animated theme
toggle from the todo list (inspired by the user's own `action-quebec.github.io`
project), a pill whose pin slides and *morphs* from a sun disc into a crescent
moon via a real CSS `d` transition between two SVG paths, not a discrete icon
swap. Colours are `conf` tokens (`--surface`/`--accent`), not a fixed palette,
after user feedback that the first draft's hardcoded sky-blue/night-navy
colours didn't fit a project's own theme. Also new: `.palette--compact`. Both
verified end-to-end on `../php-kirigami.github.io/` (real build, a headless-
Chromium click-through light/dark, screenshotted) before release — the site's
old static sun/moon icon-swap `.theme-toggle` now uses `.lightswitch`, plus a
small `margin-top` nudge for an optical text/icon alignment issue found along
the way (not a canva bug — a geometrically flex-centred box sits a touch low
next to text because of line-height leading). This release also caught up
`kirigami`/`php-prepros`/`sdk`/`plugin-highlight`/`plugin-extlink`/
`plugin-embed`'s READMEs, whose "What's new" sections had fallen behind their
actual published version by anywhere from one to six releases (`plugin-highlight`
and `plugin-embed` had none at all despite six and four real releases) —
found while doing a full README audit alongside this release.

The site itself also moved well past just the lightswitch this session — see
its own git history (`php-kirigami.github.io`): `docs/cli` rewritten with
real flags and captured terminal output (`kiri cache` was undocumented
entirely); `showcase`/`templates` got real headless-Chromium screenshots of
the three live deployments; `/design/` gained a real bug fix (`.eyebrow`
labels silently unstyled outside `.hero`/`.doc-head`) alongside more
explanation, and every remaining inline `style=""` on the site replaced with
classes; `/roadmap/` and `/changelog/` were badly out of date (several
already-shipped items still listed as open; `kiri serve`/`kiri install` had
never been announced) and got a real pass; the Part 7 deploy tutorial's
example workflow was missing the commit-back step both real templates and
this site actually ship (matters the moment a project's image pipeline
regenerates a derivative); Part 1 and Quickstart moved from `kiri watch` to
`kiri serve` for a better first-run experience. Also shipped: a real
`og:image` plus a generated favicon/apple-touch-icon, all three routed
through the image pipeline via `IMG::asset()` calls in `_layouts/header.php`
(the two icons force PNG output for the duration of their own call, since
this site's own `image.format` is webp) — `kirigami:`'s loose `image` key and
`meta.favicon`/`meta.appleTouchIcon` point at the deterministic generated
paths. Found and fixed one real papercut along the way: `injectHead()`'s
"already referenced" check (`prepros.class.php`) is a naive `str_contains()`
over the whole rendered page, so the `docs/cli` page's own captured build
output — which named the real `kirigami.core.min.css`/`.js` files — made it
think the page already had its stylesheet/script and skipped injecting them
for real; logged on the todo list rather than fixed in this release.

**Sixth release shipped (2026-09-11).** On npm and good: `@kirigami/php-prepros`
**2.0.0**, `@kirigami/kirigami` **2.0.0** (`canva`/`sdk`/`struct-walker`/
`plugin-*` unchanged, skipped by the release script). **Breaking:** `meta:`
and `jsonld:` merged into one top-level `seo:` block (`jsonld` nested inside
it) — see both packages' own rows for the mechanics. User-requested, for one
encapsulated "SEO system" instead of two blocks that happened to share
fallback data; every independent on/off switch and fallback chain is
otherwise unchanged, so the merge is purely about *where* the config lives,
not how it resolves. `../php-kirigami.github.io/` and both `../template-*/`
migrated (`meta: {}` → `seo: {}`, `jsonld:`'s content nested under it) and
floors bumped to `^2.0.0` in the same release, each verified against the
real WASM runtime before publishing (all three build clean, every `<head>`
tag and the JSON-LD graph confirmed correct post-migration).
