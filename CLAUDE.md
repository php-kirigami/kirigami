...
- **`@kirigami/plugin-highlight` 0.1.2** — `@highlight false` page opt-out +
  `nohighlight` / `language-plaintext` per-block opt-out (see the package row).
- **`@kirigami/kirigami` 1.4.2** — banner-from-template: `config.js` fills all
...
Release when ready: `npm run release` (publishes kiri 1.4.2 + plugin-highlight
0.1.2; everything else skips). Topo order: sdk → canva → struct-walker →
php-prepros → kirigami → plugin-highlight. `publish.js` also purges the jsDelivr
