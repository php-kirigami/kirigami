...
| `createProject(options)` | `{ success: true, … }` or `{ success: false, error }` | Downloads, extracts, writes metadata, starter `package.json`/`banner.txt`/`.mcp.json`/`.github/workflows/page.yml`/`.vscode/settings.json`/`.vscode/extensions.json` when missing, then optional git init + first commit. |
