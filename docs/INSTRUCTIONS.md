...
12. Package the VS Code extension per target platform with the `VSIX` workflow and publish it (see [EXTENSION-VSCODE.md](EXTENSION-VSCODE.md)).
...
The publish script scans non-private workspace packages and skips VS Code extensions (packages declaring `engines.vscode`): `kirigami-vscode` ships as platform-specific VSIX files through vsce, never to npm. See [EXTENSION-VSCODE.md](EXTENSION-VSCODE.md) for the VSIX steps. Review the selected packages and packaging contents before a release.
