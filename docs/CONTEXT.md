...
| `@kirigami/cli` | 0.1.0 | The `kiri` command — thin terminal wrapper (`build`/`export`/`watch`/`serve`/`run`/`create`/`install`/`cache`/`phpinfo`) around `@kirigami/kirigami`'s `Project` API. |
