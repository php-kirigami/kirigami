...
| `kirigami-vscode` (`packages/vscode`) | 0.1.0 | VS Code extension — a fourth "face" over `Project`, in-process like `@kirigami/cli`. v1: build/export/run/validate commands, a status-bar dev-server toggle with live build state, a `kirigami.yaml` reload watcher. Unscoped npm name (not `@kirigami/vscode`) — it doubles as the VS Code extension identity and npm workspaces symlinks by this field; see its own README for why. See [EXTENSION-VSCODE.md](EXTENSION-VSCODE.md)/[DECISIONS.md](DECISIONS.md). |
