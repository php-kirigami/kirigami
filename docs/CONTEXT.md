...
carries a `CLAUDE.md` copied verbatim from [template-CLAUDE.md](template-CLAUDE.md)
in this same `docs/` folder — authored here, never edited in the sibling.
**After changing `docs/template-CLAUDE.md`, re-copy it into every
`../template-*/CLAUDE.md`** (and the site repo). Don't confuse that file
...
| Package | Ver | Role |
|---|---|---|
| `@kirigami/kirigami` | 2.0.0 | Core engine — `Project.load()/.reload()/.validate()/.build()/.serve()/.watch()/.export()/.run()`, config loading, the plugin/task/schema system. No CLI of its own since the core-api split — `@kirigami/cli` is the terminal face. |
...
| `@kirigami/mcp` | 0.1.0 | MCP server (stdio) exposing `Project` as tools (`kirigami_config`/`validate`/`build`/`export`/`run`) for an AI agent — a third "face" over the core API, sibling to `@kirigami/cli`. No `serve`/`watch` (long-running, doesn't fit a request/response tool call). `kiri mcp` is a thin wrapper over it, same pattern as `@kirigami/cli`'s other commands. |
| `@kirigami/php-prepros` | 2.0.0 | PHP→HTML compiler + PHP class library (`PREPROS`, `MD`, `HTML`, `YAML`, `SCHEMA`, `LD`, `META`, `CACHE`, `IMG`, `FS`, `STR`, `ARR`, `CURL`, `SCRAPER`, `OBF`, `STD`). |
| `@kirigami/php-wasm` | 8.5.10-5 | Custom PHP 8.5.10 WASM build, JSPI + Node only, fork of WordPress Playground; includes Imagick. Version encodes the bundled PHP build (`8.5.10-5` → PHP 8.5.10, build 5). Also bundles a custom "Kirigami PHP extension family" (all show up in `kiri phpinfo`, none consumed by `php-prepros` yet — see [ROADMAP.md](ROADMAP.md)): `lexbor` (2.7.0, HTML5/CSS parser), `jsonk` (0.1.4, native JSON-schema validation + `json_encode`/`json_decode` replacement), `mdhtml` (0.1.2, `cmark-gfm`-backed Markdown→HTML), `navicat` (0.1.5, HTTP-tunnel DB bridge — mysql/pgsql/sqlite over libcurl). The package version hasn't been bumped for these yet — not at that step of the [workflow](INSTRUCTIONS.md) yet. |
| `@kirigami/struct-walker` | 1.0.5 | Recursive YAML/JSON walker: resolves nested file refs, converts assets to data URIs. |
| `@kirigami/sdk` | 0.2.1 | Plugin hook registry (`on`/`run`/`HOOKS`, `registerCommand`/`getCommand`/`listCommands`) + `Cache` (SQLite via `node:sqlite`). |
...
| `@kirigami/plugin-highlight` | 0.1.7 | Build-time `highlight.js` syntax highlighting — zero runtime JS shipped. |
...
- **Comments and READMEs: English.** All code comments and README prose are
  English. Deliberately still French: everything under `assets/kiri/` (an
  old standalone CLI copy, left untouched) and commit messages (Québécois).
  User-facing CLI strings are English.
- **ESM only.** Every JS package is `"type": "module"`; use
  `import`/`export`, never CommonJS. Tab indentation in the newer JS (sdk, tasks).
