...
| Package | Ver | Role |
|---|---|---|
| `@kirigami/kirigami` | 2.0.0 | Core engine — `Project.load()/.reload()/.validate()/.build()/.serve()/.watch()/.export()/.run()/.runTask()`, plus read-only `.config`/`.plugins`/`.tasks`/`.scripts` getters, config loading, the plugin/task/schema system. No CLI of its own since the core-api split — `@kirigami/cli` is the terminal face. |
