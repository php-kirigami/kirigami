...
class SCRAPER
{
    ...
    private static function scrape(string $url) {
        ...
        if(($results = $xpath->query('//meta[@name="dc.title"]')) && $results->length) $metas->title = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="dcterms.title"]')) && $results->length) $metas->title = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="twitter:title"]')) && $results->length) $metas->title = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@property="og:title"]')) && $results->length) $metas->title = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//shreddit-title')) && $results->length) $metas->title = self::getFirstItemAttr($results, 'title');
        if(($results = $xpath->query('//meta[@name="dc.description"]')) && $results->length) $metas->description = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="dcterms.description"]')) && $results->length) $metas->description = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="twitter:description"]')) && $results->length) $metas->description = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@property="og:description"]')) && $results->length) $metas->description = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="description"]')) && $results->length) $metas->description = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="twitter:image"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@property="og:image"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@name="og:image"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//img[@rel="og:image rdfs:seeAlso"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'src');
        if(($results = $xpath->query('//link[@rel="canonical"]')) && $results->length) $metas->url = self::getFirstItemAttr($results, 'href');
        if(($results = $xpath->query('//meta[@name="twitter:url"]')) && $results->length) $metas->url = self::getFirstItemAttr($results, 'content');
        if(($results = $xpath->query('//meta[@property="og:url"]')) && $results->length) $metas->url = self::getFirstItemAttr($results, 'content');

        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="twitter:app:name:iphone"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="twitter:app:name:googleplay"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="og:site_name"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@property="og:site_name"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="dc.publisher"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="DC.Publisher"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="publisher"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@property="article:author"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="author"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="twitter:creator"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="dcterms.creator"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@property="al:ios:app_name"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@property="al:android:app_name"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="DC.publisher"]')) && $results->length) $metas->label = self::getFirstItemAttr($results, 'content');
        ...
        if(empty($metas->label) && ($results = $xpath->query('//meta[@name="copyright"]')) && $results->length) $metas->label = trim(self::getFirstItemAttr($results, 'content'), '© ');
        ...
        if(($results = $xpath->query('//link[@rel="alternate" and @type="application/json+oembed" and @href]')) && $results->length) {
            if(STR::is_url(($oembedurl = self::getFirstItemAttr($results, 'href')))) {
        ...
        if(empty($metas->label)) {
            if(($results = $xpath->query('//link[@rel="manifest" and @href]')) && $results->length) {
                if($manifesturl = get_absolute_url($url, self::getFirstItemAttr($results, 'href'))) {
        ...
        if(empty($metas->label) && (($results = $xpath->query('//meta[@property="fb:page_id"]')) && $results->length)) {
            if($_contents = self::getContents('https://facebook.com/' . self::getFirstItemAttr($results, 'content'))) {
                if($xp = self::loadHTML($_contents)) {
                    if(($results = $xp->query('//meta[@property="og:title"]')) && $results->length) {
                        $metas->label = self::getFirstItemAttr($results, 'content');
                    }
                }
            }
        }
        ...
        if(empty($metas->image) && ($results = $xpath->query('//span[@class="background-image not-mobile"]')) && $results->length)
            if(preg_match('#url\(\'(.*?)\'\)#i', self::getFirstItemAttr($results, 'style'), $m))
                $metas->image = $m[1];

        if(empty($metas->image) && ($results = $xpath->query('//link[@rel="icon" and @type="image/jpeg"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'href');
        if(empty($metas->image) && ($results = $xpath->query('//link[@rel="icon" and @type="image/png"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'href');
        if(empty($metas->image) && ($results = $xpath->query('//article//img')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'src');
        if(empty($metas->image) && ($results = $xpath->query('//div[@id="image_box"]//img')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'src');
        if(empty($metas->image) && ($results = $xpath->query('//link[@rel="shortcut icon" and @type="image/jpeg" and @href]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'href');
        ...
        if(empty($metas->image) && ($results = $xpath->query('//link[@rel="apple-touch-icon"]')) && $results->length) $metas->image = self::getFirstItemAttr($results, 'href');
        ...
        if(empty($metas->label) && ($results = $xpath->query('//div[@id="elevate-global" and @data-elevate-global]')) && $results->length)
            if($data = json_decode(self::getFirstItemAttr($results, 'data-elevate-global')))
                    ...
                    if($_contents = self::getContents(str_replace('\\/', '/', $m[1])))
                        if($xp = self::loadHTML($_contents))
                            if(($results = $xp->query('//link[@rel="preload" and @as="image" and contains(@href, ".jpg")]')) && $results->length)
                                $metas->image = self::getFirstItemAttr($results, 'href');

        if(!self::url_exists($metas->image, '^image/') && ($results = $xpath->query('//img[@src]')) && $results->length) {
            foreach($results as $item) {
                /** @var DOMElement $item */
    ...
    private static function getFirstItemAttr(\Dom\Node|\Dom\NodeList $results, string $attr) {
    ...
    private static function url_exists(string $url, string|null $mimereg = null) {
